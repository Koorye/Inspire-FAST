...
class TokenizeFASTVQAInputs(DataTransformFn):
    tokenizer: _tokenizer.FASTVQATokenizer
    ...
    def __call__(self, data: DataDict) -> DataDict:
        ...
        state, actions = data["state"], data.get("actions")
        vqa = data.pop("vqa")
        tokens, token_mask, ar_mask, loss_mask = self.tokenizer.tokenize(prompt, state, vqa, actions)
        return {
            **data,
            "tokenized_prompt": tokens,
            "tokenized_prompt_mask": token_mask,
...
@dataclasses.dataclass(frozen=True)
class TokenizeFASTVQAInputsNoPad(DataTransformFn):
    tokenizer: _tokenizer.FASTVQATokenizer
    ...
    def __call__(self, data: DataDict) -> DataDict:
        ...
        state, actions = data["state"], data.get("actions")
        vqa = data.pop("vqa")
        tokens, token_mask, ar_mask, loss_mask = self.tokenizer.tokenize(prompt, state, vqa, actions, padding_to_max_len=False)
        return {
            **data,
            "tokenized_prompt": tokens,
            "tokenized_prompt_mask": token_mask,
